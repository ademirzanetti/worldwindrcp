...
import org.eclipse.plugin.worldwind.contrib.parsers.SimpleKMLParser;
	...
	implements OverlayListener
{
	...
	public TimeLoopGroundOverlay(String name, WorldWindowGLCanvas canvas ) {
		...
		super.setName(name);
		worker = new Worker((CopyOnWriteArrayList<GroundOverlayLayer>)overlays, canvas);
	}
	...
	public synchronized void setVisible (Layer layer, boolean visible) 
	{
        ...
        if ( !visible ) 
        	worker.hiddenOverlays.add((GroundOverlayLayer)layer);
        else {
        	worker.hiddenOverlays.remove((GroundOverlayLayer)layer);
        }
    }
    ...
    {	
        ...
        worker = new Worker(overlays, canvas);    	
...
//	}
	 ...
	 * @return 	Sleep interval or -1 if the worker thread has not been
	 * 			initialized.
	 */
	public long getSleepInterval () {
		if ( worker == null) return -1;
		return worker.getSleepInterval();
	}
