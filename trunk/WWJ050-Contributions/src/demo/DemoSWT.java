...
//		private TreeParent invisibleRoot;
    ...
    private IconLayer buildIconLayer()
    {
        ...
        for (double lat = 0; lat < 10; lat += 40)
        {
            for (double lon = -180; lon < 180; lon += 40)
            {
                double alt = 0;
                if (lon % 90 == 0)
                    alt = 2000000;
...
//                    new Position(Angle.fromDegrees(lat), Angle.fromDegrees(lon), alt));
                ...
                layer.addIcon(icon); //, "<b>Icon:</b><br/>" + icon.getImageSource().toString() );
            }
        }
...
//    	
//    	try {
//    		String path = "src/demo/todo/geom/mesh/loaders/sample/castle.obj";
//    		
...
//    				, 7.5e6);
//    		
//    		layer = new MeshLayer(path
//    				, MeshLayer.MESH_TYPE.WF_OBJ
//    				, pos);
//    		
//    		layer.setName(path);
//    		
...
//			invisibleRoot.addChild(new TreeParent(new NavigatorLayer(layer, null, false)));
			...
			layer = new GoogleEarthLayer(); 
			world.getModel().getLayers().add(layer);
			invisibleRoot.addChild(new TreeParent(new NavigatorLayer(layer, null, false)));

			layer = new GoogleRoadsLayer(); 
			world.getModel().getLayers().add(layer);
			invisibleRoot.addChild(new TreeParent(new NavigatorLayer(layer, null, false)));
